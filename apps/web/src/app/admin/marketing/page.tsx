...
export default function MarketingPage() {
  ...
  // Lazy-load full asset (with files) when modal opens.
  useEffect(() => {
    if (!selectedId) {
  ...
  const refreshSelected = useCallback(async () => {
    ...
    try {
      ...
      // Leave the stale data; the user can close + reopen if needed.
    }
    // Refresh the list too (variant count + updated_at + tags changes ripple here).
    loadList();
  ...
  async function handleCreate(payload: MarketingAssetCreateInput): Promise<void> {
    ...
    setCreating(false);
    await loadList();
    // Jump straight into the new asset so the team can add variants immediately.
  ...
  return (
    <div className={styles.page}>
      <PageHeader
        title="Marketing"
        subtitle="Logos, brand guide, white paper, marketing material — one curated catalog."
        count={
            ...
            <EmptyState
              ...
              message={
                <>
                  ...
                  button above, or run the seed script to import existing
                  repo assets:
                  <br />
                  <code>
                  ...
                  <br />
                  See <code>docs/MARKETING_ASSETS.md</code> for setup notes.
                </>
              }
            />
