...
// Helper functions
// Bands match the DESCRIPTIONS array: 1-3 (index 0), 4-6 (index 1), 7-10 (index 2)
...
export function computeRQ(answers: RQAnswers): RQResult {
  ...
  const A_letter = S_score >= 7 ? "S" : "R";
  const A_strength = S_score >= 7 ? S_score : 11 - S_score;
  ...
  const H_letter = L_score >= 7 ? "L" : "C";
  const H_strength = L_score >= 7 ? L_score : 11 - L_score;
